...
app.run((AnalyticsService, ErrorService, UpdateNotifierService, SelectionStateModel) => {
  ...
  const releaseType = release.getReleaseType(currentVersion);
  const shouldCheckForUpdates = UpdateNotifierService.shouldCheckForUpdates({
    ignoreSleepUpdateCheck: releaseType !== release.RELEASE_TYPE.PRODUCTION
  });
  ...
  s3Packages.getLatestVersion(releaseType).then((latestVersion) => {
    ...
    return UpdateNotifierService.notify(latestVersion, {
      allowSleepUpdateCheck: releaseType === release.RELEASE_TYPE.PRODUCTION
    });
